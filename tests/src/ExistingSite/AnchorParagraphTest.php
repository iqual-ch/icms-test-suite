 ...
 * anchors from the block list, so a paragraph type missing from the GraphQL
 * schema leaves the navigation silently empty.
 */
#[Group('icms')]
#[Group('icms_anchor')]
class AnchorParagraphTest extends IcmsExistingSiteBase {
  ...
  /**
   * Both paragraph types exist, and the anchor has its link field.
   */
  public function testParagraphTypesAndFieldsExist(): void {
    ...
    $this->assertArrayHasKey('field_icms_link', $definitions, 'The anchor paragraph has field_icms_link, its title becomes the anchor id.');
  }
